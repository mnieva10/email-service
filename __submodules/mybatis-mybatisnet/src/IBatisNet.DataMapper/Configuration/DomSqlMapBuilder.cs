...
namespace IBatisNet.DataMapper.Configuration
{
    ...
    public class DomSqlMapBuilder
    {
        ...
        /// Token for xml path to global type alias elements.
        /// </summary>
        private const string XML_GLOBAL_MAPMODULE = "sqlMapConfig/sqlMapModules/sqlMapModule";
        ...
        /// <summary>
        /// Token for xml path to module sqlMap elements.
        /// </summary>
        private const string XML_MODULE_MAP = "sqlMapModule/sqlMaps/sqlMap";
            ...
            bool useConfigFileWatcher, bool isCallFromDao)
        {
            ...
            _configScope.XmlNamespaceManager.AddNamespace(MODULE_NAMESPACE_PREFIX, MODULE_XML_NAMESPACE);
        ...
        private void Initialize()
        {
            ...
            foreach (XmlNode xmlNode in _configScope.SqlMapConfigDocument.SelectNodes(ApplyDataMapperNamespacePrefix(XML_GLOBAL_MAPMODULE), _configScope.XmlNamespaceManager))
            {
                _configScope.NodeContext = xmlNode;
                ConfigureSqlMapModule();
            }
            _configScope.ErrorContext.Reset();
            #endregion
        ...
        private void ConfigureSqlMapModule()
        {
            XmlNode sqlModuleNode = _configScope.NodeContext;

            _configScope.ErrorContext.Activity = "loading SqlMap Module";
            _configScope.ErrorContext.Resource = sqlModuleNode.OuterXml.ToString();
            if (_configScope.UseConfigFileWatcher)
            {
                if (sqlModuleNode.Attributes["resource"] != null || sqlModuleNode.Attributes["url"] != null)
                {
                    ConfigWatcherHandler.AddFileToWatch(Resources.GetFileInfo(Resources.GetValueOfNodeResourceUrl(sqlModuleNode, _configScope.Properties)));
                }
            }

            _configScope.SqlMapModuleDocument = Resources.GetAsXmlDocument(sqlModuleNode, _configScope.Properties);
            
            if (_configScope.ValidateSqlMap)
            {
                ValidateSchema(_configScope.SqlMapModuleDocument.ChildNodes[1], "SqlMapModule.xsd");
            }
            foreach (XmlNode xmlNode in _configScope.SqlMapModuleDocument.SelectNodes(ApplyModuleNamespacePrefix(XML_MODULE_MAP), _configScope.XmlNamespaceManager))
            {
                _configScope.NodeContext = xmlNode;
                ConfigureSqlMap();
                _configScope.NodeContext = sqlModuleNode;
            }
        }
        ...
        public Stream GetStream( string schemaResourceKey )
        {
            ...
            return assembly.GetManifestResourceStream(string.Format("{0}.Resources.{1}", assembly.GetName().Name, schemaResourceKey));  
        }
        ...
        /// <summary>
        /// Apply the module namespace prefix
        /// </summary>
        /// <param name="elementName"></param>
        /// <returns></returns>
        public static string ApplyModuleNamespacePrefix( string elementName )
        {
            return MODULE_NAMESPACE_PREFIX+ ":" + elementName.
                Replace("/", "/" + MODULE_NAMESPACE_PREFIX + ":");
        }
        /// <summary>
        /// Apply the module namespace prefix
        /// </summary>
